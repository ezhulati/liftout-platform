...
// Compact opportunity card for tablet
...
// Mini opportunity card for phone mockup (extra compact)
function OpportunityCardMini({ opportunity }: { opportunity: typeof mockOpportunities[0] }) {
  return (
    <div className="bg-white rounded-md p-2 border border-gray-200 shadow-sm">
      <div className="flex items-center gap-2">
        {/* Company logo - smaller */}
        <div className={`w-7 h-7 ${opportunity.logoColor} rounded-md flex items-center justify-center text-white font-bold text-[10px] flex-shrink-0`}>
          {opportunity.logo}
        </div>
        <div className="flex-1 min-w-0">
          <div className="flex items-center gap-1">
            <h4 className="font-semibold text-gray-900 text-[11px] truncate">{opportunity.company}</h4>
            {opportunity.urgent && (
              <span className="px-1 py-0.5 bg-amber-100 text-amber-700 text-[8px] font-medium rounded">
                Urgent
              </span>
            )}
          </div>
          <p className="text-[10px] text-gray-600 truncate">{opportunity.title}</p>
        </div>
      </div>
    </div>
  );
  ...
  return (
    <div className="relative">
      ...
      <div className="block md:hidden">
        {/* Constrained phone frame - 280px width, ~500px height for realistic proportions */}
        <div className="bg-gray-900 rounded-[2rem] p-1.5 shadow-2xl w-[260px] mx-auto relative">
          ...
          <div className="bg-white rounded-[1.5rem] overflow-hidden relative">
            {/* Status bar - compact */}
            <div className="bg-gray-50 px-4 pt-7 pb-1 flex justify-between items-center text-[10px] text-gray-500">
              <span className="font-medium">9:41</span>
              <span className="font-semibold text-gray-700">Liftout</span>
              <div className="flex items-center gap-0.5">
                <div className="w-5 h-2 border border-gray-400 rounded-sm flex items-center p-px">
            ...
            <div className="px-3 py-2 border-b border-gray-100">
              <h3 className="font-bold text-gray-900 text-xs">Opportunities</h3>
            </div>

            {/* Search - compact */}
            <div className="px-3 py-1.5">
              <div className="bg-gray-100 rounded-md px-2 py-1.5 text-[10px] text-gray-400 flex items-center gap-1.5">
                <svg className="w-3 h-3" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M21 21l-6-6m2-5a7 7 0 11-14 0 7 7 0 0114 0z" /></svg>
                Search opportunities...
              </div>
            </div>

            {/* Results - show only 3 cards */}
            <div className="px-3 py-1.5">
              ...
              <div className="space-y-1.5">
                {mockOpportunities.slice(0, 3).map((opportunity) => (
                  <OpportunityCardMini key={opportunity.company} opportunity={opportunity} />
                ))}
              </div>
            </div>
